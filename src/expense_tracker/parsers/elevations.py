"""Elevations Credit Union checking account CSV parser.

Elevations CSV format:
    Transaction ID, Posting Date, Effective Date, Transaction Type, Amount,
    ..., Description, ...

Sign convention:
    Negative amounts are debits (expenses/outflows).
    Positive amounts are credits (income/inflows/refunds).

This parser uses Posting Date. The bank's own "Transaction ID" column is
ignored -- we generate our own deterministic IDs.
"""
...
EXPECTED_COLUMNS = {"Posting Date", "Transaction Type", "Amount", "Description"}


def parse(file_path: Path, institution: str, account: str) -> StageResult:
    """Parse an Elevations Credit Union CSV file into normalized Transactions.

    Args:
        file_path: Path to the CSV file.
        institution: Institution key, e.g. "elevations".
